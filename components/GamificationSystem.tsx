        ...
        {newBadges.map(badge => (
          ...
          >
            <div className="bg-gradient-to-br from-metallicGold-500/20 to-metallicGold-900/20 backdrop-blur-sm border border-metallicGold-500/40 rounded-2xl p-8 text-center">
              ...
              <h4 className="text-xl font-semibold text-offWhite-200 mb-2">
                {badge.name}
              </h4>
...
// 연속 학습 일수 추적
export function StreakTracker({ currentStreak = 5, bestStreak = 12 }: {
  ...
  const getStreakColor = (days: number) => {
    if (days >= 30) {return 'from-purple-500 to-indigo-500'}
  ...
  const getStreakTitle = (days: number) => {
    if (days >= 30) {return '레전드 학습자'}
    if (days >= 14) {return '학습 마스터'}
    if (days >= 7) {return '꾸준함의 달인'}
    if (days >= 3) {return '성실한 학습자'}
    return '시작하는 학습자'
  }
          ...
          >
            <div className="flex items-start justify-between mb-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h4 className="font-semibold text-offWhite-200">
                    {goal.title}
                  </h4>
              ...
              <div className="flex items-center gap-4 mb-3">
                ...
                <div>
                  <div className="text-2xl font-bold text-offWhite-200">
                    {achievement.value}
                    <span className="text-sm text-offWhite-600 ml-1">
                      {achievement.unit}
                    </span>
                  </div>
