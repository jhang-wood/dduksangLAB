...
import { Plus, Edit, Trash2, Eye, EyeOff, Save, X, ChevronDown, ChevronUp } from 'lucide-react';
...
export default function AdminLecturesPage() {
  ...
  const handleUpdateLecture = async () => {
    ...
    try {
      ...
      await supabase.from('lecture_chapters').delete().eq('lecture_id', editingLecture.id);
      ...
      if (chapters.length > 0) {
        ...
        await supabase.from('lecture_chapters').insert(chaptersData);
      }
  ...
  const handleDeleteLecture = async (lectureId: string) => {
    if (
      !userNotification.confirm(
        '정말 이 강의를 삭제하시겠습니까? 모든 챕터와 수강 기록이 삭제됩니다.'
      )
    ) {
      return;
    }
  ...
  const updateArrayField = (
    field: 'objectives' | 'requirements' | 'target_audience',
    index: number,
    value: string
  ) => {
  ...
  const ChapterManagement = ({
    ...
    const deleteChapter = async (chapterId: string) => {
      ...
      try {
        const { error } = await supabase.from('lecture_chapters').delete().eq('id', chapterId);
        ...
        <div className="mb-6 p-4 bg-deepBlack-900 rounded-lg">
          ...
          <div className="grid grid-cols-2 gap-3 mb-3">
            ...
            <input
              ...
              onChange={e =>
                setNewChapter({ ...newChapter, duration: parseInt(e.target.value) ?? 0 })
              }
              className="px-3 py-2 bg-deepBlack-600 border border-metallicGold-900/30 rounded text-offWhite-200 focus:outline-none focus:ring-2 focus:ring-metallicGold-500"
            />
          </div>
            ...
            lectureChapters.map(chapter => (
              <div key={chapter.id} className="p-4 bg-deepBlack-900 rounded-lg">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      onClick={() =>
                        setEditingChapter(
                          editingChapter === chapter.id ? null : (chapter.id ?? null)
                        )
                      }
                ...
                <div className="text-sm text-offWhite-600 mb-2">
                  <span className="mr-4">
                    재생시간: {Math.floor((chapter.duration ?? 0) / 60)}분{' '}
                    {(chapter.duration ?? 0) % 60}초
                  </span>
                  {chapter.is_preview && (
                ...
                <div className="text-xs text-offWhite-700">비디오: {chapter.video_url}</div>
                  ...
                  <div className="mt-3 pt-3 border-t border-metallicGold-900/30">
                    <div className="grid grid-cols-2 gap-3 mb-3">
                      <input
                        ...
                        onBlur={e =>
                          void updateChapter(chapter.id ?? '', { title: e.target.value })
                        }
                        className="px-3 py-2 bg-deepBlack-600 border border-metallicGold-900/30 rounded text-offWhite-200 focus:outline-none focus:ring-2 focus:ring-metallicGold-500"
                      />
                      <input
                    ...
                    <input
                      ...
                      onBlur={e =>
                        void updateChapter(chapter.id ?? '', { video_url: e.target.value })
                      }
                      className="w-full px-3 py-2 bg-deepBlack-600 border border-metallicGold-900/30 rounded text-offWhite-200 focus:outline-none focus:ring-2 focus:ring-metallicGold-500 mb-3"
                    />
                    <textarea
      ...
      <div className="container mx-auto px-4 pt-24 pb-12">
        ...
        {lectures.length === 0 && (
          <div className="text-center py-20">
            ...
            <p className="text-offWhite-600">새 강의를 추가하여 시작하세요</p>
          </div>
        )}
      </div>
          ...
          >
            ...
            <div className="p-6 space-y-6">
                  ...
                  <div key={index} className="p-4 bg-deepBlack-600 rounded-lg space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <input
                        ...
                        onChange={e =>
                          updateChapter(index, 'duration', parseInt(e.target.value) ?? 0)
                        }
                        className="px-3 py-2 bg-deepBlack-900 border border-metallicGold-900/30 rounded text-offWhite-200 focus:outline-none focus:ring-2 focus:ring-metallicGold-500"
                        placeholder="재생시간 (초)"
                      />
              ...
              <div className="flex justify-end gap-4 pt-6 border-t border-metallicGold-900/30">
                ...
                <button
                  onClick={() =>
                    void (editingLecture ? handleUpdateLecture() : handleCreateLecture())
                  }
                  className="px-6 py-3 bg-gradient-to-r from-metallicGold-500 to-metallicGold-900 text-deepBlack-900 rounded-lg font-semibold hover:from-metallicGold-400 hover:to-metallicGold-800 transition-all flex items-center gap-2"
                >
                  <Save size={20} />
