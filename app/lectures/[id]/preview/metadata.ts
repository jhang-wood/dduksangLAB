...
export async function generateLectureMetadata(id: string): Promise<Metadata> {
  try {
    ...
    const description = lecture.description ?? `${lecture.title} 강의를 dduksangLAB에서 만나보세요. ${lecture.instructor_name} 강사님의 전문적인 강의로 실무 역량을 키워보세요.`
    ...
    return {
      ...
      twitter: {
        ...
        title,
        description,
        images: lecture.thumbnail_url ? [lecture.thumbnail_url] : ['https://dduksanglab.com/og-image.jpg'],
        creator: '@dduksangLAB',
      },
      robots: {
        index: true,
