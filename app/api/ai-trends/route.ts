...
export async function POST(request: NextRequest) {
  try {
    ...
    logger.log(
      ...
      trendError ? 'ERROR' : 'SUCCESS',
      trendError || 'Trend created'
    );
    ...
    const { error: hashError } = await adminClient.from('ai_trends_hash').insert({
    ...
    if (error && typeof error === 'object') {
      ...
      if ('details' in error) {
        errorDetails = { ...errorDetails, details: error.details };
      }
      if ('hint' in error) {
        errorDetails = { ...errorDetails, hint: error.hint };
      }
    }
