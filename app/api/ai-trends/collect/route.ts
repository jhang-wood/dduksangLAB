...
// AI trend categories
const TREND_CATEGORIES = ['AI 기술', 'AI 도구', 'AI 활용', 'AI 비즈니스', 'AI 교육'];
...
export async function POST(request: NextRequest) {
  try {
    ...
    for (const trendData of aiTrends) {
      try {
        ...
        if (!existingHash) {
          ...
          if (!trendError && trend) {
            // Store content hash
            await supabase.from('ai_trends_hash').insert({
    ...
    return NextResponse.json({ error: 'Failed to collect AI trends' }, { status: 500 });
  }
}
...
function generateIntelligentAITrends(count: number) {
  ...
  const trendTopics = [
    {
      ...
      baseContent:
        'AI 도구 시장은 빠르게 발전하고 있으며, 매달 새로운 혁신적인 도구들이 등장하고 있습니다.',
    ...
    },
    {
      ...
      baseContent:
        '대규모 언어모델 분야는 지속적인 발전을 보이고 있으며, 각 모델마다 고유한 강점을 보여주고 있습니다.',
    ...
    },
    {
      ...
      baseContent:
        'AI 스타트업 생태계는 지속적인 성장을 보이고 있으며, 다양한 분야에서 혁신적인 솔루션들이 개발되고 있습니다.',
    ...
    },
    {
      ...
      baseContent:
        '노코드 AI 플랫폼의 등장으로 비개발자도 쉽게 AI 솔루션을 구축할 수 있게 되었습니다.',
    ...
    },
    {
      ...
      baseContent:
        'AI 기술의 발전은 전통적인 직업군에 변화를 가져오는 동시에 새로운 기회를 창출하고 있습니다.',
    ...
    },
    {
      ...
      baseContent:
        '멀티모달 AI는 다양한 형태의 데이터를 통합적으로 처리하여 더욱 자연스러운 인터랙션을 가능하게 합니다.',
    ...
    },
    {
      ...
      baseContent:
        '개인화된 AI 서비스는 사용자의 데이터를 학습하여 점점 더 정확하고 유용한 추천과 지원을 제공합니다.',
  ...
  // 요청된 수만큼 트렌드 선택
  const selectedTopics = trendTopics.sort(() => Math.random() - 0.5).slice(0, count);
