...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!payapp.verifyWebhook(data as Record<string, unknown>, signature)) {
      return NextResponse.json({ error: '잘못된 요청입니다' }, { status: 401 });
    }
    ...
    if (updateError) {
      ...
      return NextResponse.json({ error: '결제 정보 업데이트 실패' }, { status: 500 });
    }
    ...
    return NextResponse.json({ error: '콜백 처리 중 오류가 발생했습니다' }, { status: 500 });
  }
}
